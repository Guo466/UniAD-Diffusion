...
$$
\begin{aligned}
\underset{x_{0:N},\, u_{0:N-1}}{\text{minimize}} \quad & l_f(x_N) + \sum_{k=0}^{N-1} l(x_k, u_k) \\
\text{subject to} \quad & x_{k+1} = f(x_k, u_k) &&\text{动力学约束（等式）} \\
& g_k(x_k, u_k) \le 0 &&\text{不等式约束（比如车道边界、避障）} \\
& h_k(x_k, u_k) = 0 &&\text{等式约束（比如终点必须精确到达）}
\end{aligned}
$$
...
$$
x = [\text{位置}x,\ \text{位置}y,\ \text{朝向角}\theta,\ \text{曲率}\kappa,\ \text{曲率变化率}\dot{\kappa}]
$$
...
$$
u = [\ddot{\kappa}]\quad(\text{曲率的二阶导数，也就是“曲率变化率的变化率”})
$$
...
$$
\dot{x}(t) = f_c(x(t), u(t)) \qquad \text{（连续时间动力学）}
$$

或离散形式：

$$
x_{k+1} = f(x_k, u_k) \qquad \text{（离散时间动力学，每隔 } \Delta t \text{ 采样一次）}
$$
...
\dot{\theta} &= \kappa \\
\dot{\kappa} &= \kappa' \\
\dot{\kappa}' &= u \quad (\text{即 } \ddot{\kappa})
\end{aligned}
$$

> 记号说明：状态里除了曲率 $\kappa$ 外，还单独保留了"曲率变化率"这个状态分量（代码里叫 `dκ`，这里记作 $\kappa'$），$\kappa'$ 自身的变化率（即 `ddκ`）才是控制量 $u$。
...
**离散化方法**：连续时间的 $\dot{x} = f_c(x,u)$ 要变成离散的 $x_{k+1} = f(x_k, u_k)$，中间需要"积分器"。最简单的是前向欧拉：

$$
x_{k+1} \approx x_k + f_c(x_k, u_k) \cdot \Delta t
$$

更精确的是四阶龙格库塔（RK4），用 4 个采样点加权平均斜率，精度是欧拉法的 4 阶（误差量级 $O(\Delta t^5)$ vs $O(\Delta t^2)$）：

$$
\begin{aligned}
k_1 &= f_c(x_k, u_k) \\
k_2 &= f_c(x_k + 0.5\Delta t\, k_1, u_k) \\
k_3 &= f_c(x_k + 0.5\Delta t\, k_2, u_k) \\
k_4 &= f_c(x_k + \Delta t\, k_3, u_k) \\
...
> 给定初始状态 $x_0$，找一串控制量 $u_0, u_1, \ldots, u_{N-1}$（进而通过动力学方程滚出状态序列 $x_0, x_1, \ldots, x_N$），使得总代价 $\sum l(x_k,u_k) + l_f(x_N)$ 最小，同时满足所有约束。

这是一个典型的**非线性、高维、带约束的优化问题**。直接用通用求解器（梯度下降/牛顿法）很难处理，因为：

1. 变量维度高（$N$ 个时间步 × 状态维度）；
2. 动力学约束是非线性的（$x_{k+1} = f(x_k, u_k)$ 里 $f$ 通常非线性，比如上面的 $\cos(\theta)$）；
3. 还有额外的不等式约束（避障、限速）。
...
对于一个光滑函数 $f(x)$，在点 $x_0$ 附近可以展开为：

$$
f(x_0+\delta) \approx f(x_0) + f'(x_0)\,\delta + \frac{1}{2} f''(x_0)\,\delta^2
$$

- 一次项 $f'(x_0)$ 告诉你函数在这一点"往哪个方向变化最快"（梯度/导数）；
- 二次项 $f''(x_0)$ 告诉你这个变化趋势"弯曲"得多厉害（曲率/二阶导数，也叫 Hessian 的一维版本）。
...
$$
\begin{aligned}
f_x &\equiv \left.\frac{\partial f(x,u)}{\partial x}\right|_{x_k,u_k} && \text{一阶偏导（梯度/雅可比）} \\
f_{xx} &\equiv \left.\frac{\partial^2 f(x,u)}{\partial x^2}\right|_{x_k,u_k} && \text{二阶偏导（Hessian）} \\
f_{xu} &\equiv \left.\frac{\partial^2 f(x,u)}{\partial x\,\partial u}\right|_{x_k,u_k} && \text{交叉二阶偏导}
\end{aligned}
$$
...
$$
f(x_k+\delta x,\, u_k+\delta u) \approx f(x_k,u_k) + f_x\,\delta x + f_u\,\delta u + \frac{1}{2}
\begin{bmatrix}\delta x \\ \delta u\end{bmatrix}^{\!\top}
\begin{bmatrix} f_{xx} & f_{xu} \\ f_{xu}^\top & f_{uu} \end{bmatrix}
\begin{bmatrix}\delta x \\ \delta u\end{bmatrix}
$$

（其中 $f_x\,\delta x + f_u\,\delta u$ 是一次项，最后的二次型是二次项。）
...
变量名 `l_x, l_xx, l_u, l_uu, l_ux, f_x, f_u` 与论文记号 $l_x, l_{xx}, \ldots$ 完全一一对应，这是刻意为了方便对照论文而保留的命名风格（代码注释里也明说了 "We follow the variable namings in the paper"）。
...
对轨迹优化问题，"阶段"就是时间步 $k=0,1,\ldots,N$。定义**最优代价函数（cost-to-go / value function）**：

$$
V_k(x) \triangleq \text{从状态 } x \text{ 出发，走完剩下所有步骤（} k \text{ 到 } N\text{），能达到的最小总代价}
$$
...
$$
\begin{aligned}
V_N(x_N) &= l_f(x_N) && \text{式(4) 终端条件} \\
V_k(x_k) &= \min_{u_k} \big\{\, l(x_k,u_k) + V_{k+1}(f(x_k,u_k)) \,\big\} && \text{式(5)} \\
&= \min_{u_k} Q_k(x_k,u_k) && \text{式(6)}
\end{aligned}
$$
...
$$
\delta V_k(x_k) \approx p_k^\top \delta x_k + \frac{1}{2}\, \delta x_k^\top P_k\, \delta x_k
$$
...
如果动力学是**线性**的（$x_{k+1} = A_k x_k + B_k u_k$），代价是**二次**的（$l = x^\top Q x + u^\top R u$），那么这个问题叫"线性二次调节器"问题（LQR），它有**精确解析解**——不需要迭代，直接反向递推一遍 Riccati 方程就能得到全局最优解。

但现实中的动力学几乎总是非线性的（比如 clothoid 动力学里的 $\cos(\theta)$）。iLQR 的想法是：
...
$$
p_N = \left.\frac{\partial l_f(x)}{\partial x}\right|_{x_N} \ (\text{终端代价的梯度}), \qquad
P_N = \left.\frac{\partial^2 l_f(x)}{\partial x^2}\right|_{x_N} \ (\text{终端代价的 Hessian})
$$
...
+
\begin{bmatrix}Q_x \\ Q_u\end{bmatrix}^{\!\top}
\begin{bmatrix}\delta x_k \\ \delta u_k\end{bmatrix}
$$
...
$$
\begin{aligned}
Q_{xx} &= l_{xx} + A_k^\top P_{k+1} A_k \\
Q_{uu} &= l_{uu} + B_k^\top P_{k+1} B_k \\
Q_{ux} &= l_{ux} + B_k^\top P_{k+1} A_k \\
Q_x &= l_x + A_k^\top p_{k+1} \\
Q_u &= l_u + B_k^\top p_{k+1}
\end{aligned}
$$

这里 $A_k = \partial f/\partial x$，$B_k = \partial f/\partial u$ 是动力学在当前点的雅可比（一阶导数），可以理解为"把下一步的 cost-to-go 二次型，通过线性化的动力学，拉回到当前时刻的状态-控制空间里"。

**直觉理解**：$Q_{uu}$ 告诉你"在这一步稍微改变一点控制量 $u$，对总代价的二阶影响有多大"（越大说明这个方向的控制对代价越敏感）；$Q_{ux}$ 告诉你"状态的扰动和控制的扰动如何耦合影响代价"。

**求最优控制修正量**：对 $\delta Q_k$ 关于 $\delta u_k$ 求最小值（这是一个无约束二次函数求最小值，直接令梯度为零），得到（论文式 16）：
...
- $d_k = -(Q_{uu}+\rho I)^{-1} Q_u$：**前馈项**（feedforward），不依赖 $\delta x_k$，是"标称"的修正量；
- $K_k = -(Q_{uu}+\rho I)^{-1} Q_{ux}$：**反馈增益矩阵**（feedback gain），乘以状态偏差 $\delta x_k$ 后加到控制上，起到"实时纠偏"的作用（类似 PID 里的比例反馈）。

$\rho I$ 是**正则化项**（regularization），加上它是为了保证 $Q_{uu}+\rho I$ 一定可逆（数值稳定性），$\rho$ 太小可能矩阵接近奇异，太大又会让每步更新过于保守。代码里对应 `mu`（即 `Q_uu_reg`）。

把 $\delta u_k^{*}$ 代回 $\delta Q_k$，可以解出当前时刻的 $p_k, P_k$（论文式 17、18）：
...
一一对应关系：$V_x \leftrightarrow p_k$, $V_{xx} \leftrightarrow P_k$, `meta->k_trajectory`$\leftrightarrow d_k$（前馈）, `meta->K_trajectory`$\leftrightarrow K_k$（反馈增益）, `meta->mu`$\leftrightarrow \rho$（正则化）。

注意代码用的是 `mu` 命名（来自另一篇经典论文 "Control-Limited DDP"），而 ALTRO 论文里用 $\rho$，两者是同一个东西——数值稳定性正则化系数。
...
$$
\bar{u}_k = u_k + K_k(\bar{x}_k - x_k) + \alpha\, d_k \qquad \text{（论文 Algorithm 3 第4行）}
$$

$$
\bar{x}_{k+1} = f(\bar{x}_k, \bar{u}_k) \qquad \text{（用真实非线性动力学滚出下一状态）}
$$

注意这里：

- $\bar{x}_k - x_k$ 是"新旧轨迹在这一步的状态偏差"，$K_k$ 把这个偏差转换为控制的反馈修正——这保证了新轨迹即使跟旧轨迹有偏差，依然能沿着"最优反馈策略"走，而不是死板地套用旧的控制量；
- $\alpha$ 是**线搜索步长**（line search step size），从 $1$ 开始尝试，如果这一步导致代价没有充分下降，就把 $\alpha$ 减小再试（比如乘 0.5），直到代价确实下降为止。这保证了 iLQR 的**全局收敛性**（每次迭代代价单调不增）。
- 用的是**真实的非线性动力学** $f(\bar{x}_k, \bar{u}_k)$ 来滚出新轨迹（而不是线性化后的近似），这保证了每一步迭代后的轨迹永远是"动力学可行"的——这也是间接法相比直接法的天然优势（"anytime dynamically feasible"，论文反复强调的一点）。
...
`meta.expected_cost_improvement = -α(ΔV_0 + αΔV_1)` 正是论文式 (19) $\Delta V_k = d_k^\top Q_u + \frac{1}{2}d_k^\top Q_{uu} d_k$ 在乘以步长 $\alpha$ 之后的形式（因为 $d_k$ 要乘 $\alpha$ 才是实际施加的修正量，二次项要乘 $\alpha^2$）——**这一步是判断"这次改进是否符合预期"的关键**，如果实际代价下降比预期差太多（`cost_improvement_ratio` 太小），就认为这个二次近似不可信，需要缩小步长重试，或增大正则化 `mu` 重新做反向传播。
...
用数学符号表示即：给定初始状态 $x_0$、初始控制序列 $U$ 与容差 `tolerance`，先滚出初始状态轨迹 $X$；再反复执行：计算当前总代价 $J$，调用反向传播得到 $K, d, \Delta V$，调用前向传播更新 $X, U, J$，直到 $|J - J_{prev}| \leq \text{tolerance}$（代价不再显著下降）为止。
...
这是一种"自适应步长"策略：如果这次迭代成功（代价确实下降了），就减小正则化 `mu`（下次更信任二次近似，走大步）；如果失败了（$Q_{uu}$ 不正定或线搜索找不到合适步长），就增大 `mu`（相当于把问题变得更像梯度下降，更保守但更稳）。这跟 Levenberg-Marquardt 算法里阻尼因子的自适应调整逻辑是同一套思想。
...
一种朴素想法：把约束 $c(x)\le 0$ 变成惩罚项 $\mu\cdot\max(0,c(x))^2$ 加到代价里，$\mu$ 取一个很大的数，这样违反约束时代价会暴涨，优化器自然会避开。这叫**纯罚函数法（penalty method）**。

问题：$\mu$ 太小则约束满足不精确（优化器会为了降低其他代价，"稍微"违反约束换取更低的总代价）；$\mu$ 太大则代价函数变得病态（数值上极度敏感，一阶/二阶导数在惩罚区域附近爆炸式增长，导致优化器数值不稳定，收敛困难）。这是纯罚函数法的经典缺陷。

### 7.2 增广拉格朗日法的改进

增广拉格朗日（Augmented Lagrangian, AL）法巧妙地结合了"拉格朗日乘子法"和"罚函数法"，用一个**逐渐更新的乘子 $\lambda$** 来避免把 $\mu$ 无限增大。原问题（论文式 20）：

$$
\begin{aligned}
\text{minimize} \quad & f(x) \\
\text{subject to} \quad & c_I(x) \le 0,\ \ c_E(x) = 0
\end{aligned}
$$
...
$$
L_A(x,\lambda,\mu) = f(x) + \lambda^\top c(x) + \frac{1}{2}\, c(x)^\top I_\mu\, c(x)
$$

拆开看：

- $f(x)$：原始代价；
- $\lambda^\top c(x)$：**一阶拉格朗日项**，$\lambda$ 是拉格朗日乘子（可正可负，等式约束下），作用是"精确地"把约束的影子价格编码进代价里；
- $\frac{1}{2}c(x)^\top I_\mu c(x)$：**二阶罚项**，$\mu$ 是罚参数，作用是让违反约束的代价随违反程度平方增长，帮助数值稳定并加速收敛。

对于不等式约束，$I_\mu$ 是一个对角矩阵，定义为（论文式 22）：

$$
I_{\mu,ii} =
\begin{cases}
0, & c_i(x) < 0 \text{ 且 } \lambda_i = 0 \quad \text{（约束未激活，且乘子也是0，不需要惩罚）} \\
\mu_i, & \text{否则（约束被违反，或虽然满足但乘子非零——说明该约束在边界上“起作用”）}
\end{cases}
$$
...
- **内层**：固定 $\lambda, \mu$，用某个求解器（这里就是 iLQR）最小化 $L_A(x,\lambda,\mu)$，得到近似最优解 $\hat{x}^{*}$；
- **外层**：根据 $\hat{x}^{*}$ 更新 $\lambda$ 和 $\mu$，再重新做内层优化，如此反复，直到约束违反量足够小。
...
\lambda_i^{\text{new}} =
\begin{cases}
\lambda_i + \mu_i\, c_i(\hat{x}^{*}), & \text{对等式约束 } i \in E \\
\max\big(0,\ \lambda_i + \mu_i\, c_i(\hat{x}^{*})\big), & \text{对不等式约束 } i \in I \text{（保证乘子非负）}
\end{cases}
$$

**直觉理解**：如果约束被违反（$c_i(\hat{x}^{*}) > 0$），说明惩罚力度不够，下一轮就把乘子调大，加重惩罚；如果约束满足得很好（$c_i(\hat{x}^{*})$ 很负），且当前乘子已经是 0，$\max(0,\ldots)$ 会让它保持在 0（不需要惩罚一个本来就没问题的约束）。

罚参数更新（论文式 24，单调递增的调度策略）：

$$
\mu_i^{\text{new}} = \varphi_i \cdot \mu_i \qquad (\varphi_i > 1,\ \text{比如取 } 10)
$$

每轮外层迭代都把罚参数放大 $\varphi$ 倍，让约束惩罚越来越"严格"。这个单调递增的策略保证了理论上的收敛性，但正如论文所说，"AL methods make rapid initial progress, but suffer from slow constraint convergence once the penalty is capped at a maximum finite value"——这也是论文后面要引入"投影牛顿法作为第二阶段"的动机（第 9.4 节）。
...
用数学符号表示即：初始化 $\lambda, \mu, \varphi$，在 $\max(c) > \text{tolerance}$ 时反复用 iLQR 最小化 $L_A(x,\lambda,\mu)$，再按式 (23) 更新 $\lambda$、按式 (24) 更新 $\mu$，最终返回解 $X$ 与乘子 $\lambda$。
...
`scale` 就是论文里的 $\varphi$，对应配置项 `penalty_scaling_factor`（默认值可以在 `ALSolverConfig` 里看到，注释里明确写着 "\phi in ALTRO paper"）：
...
**终止条件判断**（对应论文 $\max(c) > \text{tolerance}$，但工程上还要加上"迭代次数、超时、罚参数超上限"等失败保护）：
...
用数学符号表示即：初始化 $x_0, U, \text{tolerance}$ 及期望轨迹 $\tilde{X}$；若采用不可行初始化则 $X \leftarrow \tilde{X}$ 并由式 (37) 计算虚拟控制 $W$，否则用 $U$ 滚出可行轨迹 $X$；第一阶段 $(X,U), \lambda \leftarrow \text{AL\_iLQR}((X,U), \text{ILQR}, \text{tol.})$；可选的第二阶段 $(X,U) \leftarrow \text{ProjectedNewton}((X,U,\lambda), \text{tol.})$；最终返回 $X, U$。
...
值得注意的是：`clothoid_path_optimizer` **没有实现论文的第二阶段（投影牛顿法/Algorithm 5）**，只用了第一阶段（AL + iLQR）。这是工程上的合理取舍——车辆路径规划对约束满足精度的要求（比如 `constraint_tolerance`）远没有论文里机械臂避障场景（要求 $10^{-8}$）那么苛刻，$10^{-4}$ 量级的容差配合足够的安全 buffer 已经足够安全；而且投影牛顿法需要显式构造大规模稀疏 KKT 矩阵求逆，在车载实时计算资源上代价过高。这一点会在第 9.4 节详细展开原因。
...
对照论文公式 (1) 的记号：$x_k = [x,y,\theta,\kappa,\dot{\kappa}]^\top \in \mathbb{R}^5$（对应论文的 $x\in\mathbb{R}^n$），$u_k = [\ddot{\kappa}] \in \mathbb{R}^1$（对应论文的 $u\in\mathbb{R}^m$）。`C1`（等式约束数）和 `C2`（不等式约束数）在本工程里都取 `Eigen::Dynamic`，因为不同路径规划场景下约束数量是动态变化的（障碍物数量、车道边界数量都不固定）。
...
值得注意：`clothoid_path_optimizer` 在实践中**没有使用等式约束**（`equality_constraints` 全部是空指针），所有硬性限制（车道边界、避障、曲率范围）都建模成不等式约束 $g_k(x_k,u_k)\le 0$。这在实际工程中很常见——因为等式约束往往可以放宽为"落在一个很窄区间内"的不等式约束，这样数值上更鲁棒（避免因为浮点误差导致等式约束永远无法精确满足）。
...
**问题**：AL 方法要让约束收敛得快，罚参数 $\mu$ 必须变得很大（比如 $10^8$）。但 $Q_{uu}$ 里包含了 $\mu$ 相关的二次惩罚项，$\mu$ 太大会导致 $Q_{uu}$ 的**条件数**（最大特征值/最小特征值）急剧恶化，进而在矩阵求逆（$Q_{uu}^{-1}$）时产生严重的数值误差（甚至变成负定，导致 Cholesky 分解失败）。

**解决方案**：不直接对 $P_k, Q_{xx}, Q_{uu}$ 这些矩阵做数值运算，而是维护它们的 **Cholesky 平方根因子**（$S=\sqrt{P},\ Z_{xx}=\sqrt{Q_{xx}},\ Z_{uu}=\sqrt{Q_{uu}}$），所有递推关系都改写成对这些"平方根"矩阵的运算（用 QR 分解代替直接矩阵乘法/求逆）。这是数值线性代数里的经典技巧（源自"平方根卡尔曼滤波器"），核心好处是：**矩阵的条件数在平方根空间里是原来的平方根**（比如原矩阵条件数是 $10^{16}$，平方根矩阵的条件数只有 $10^8$），大幅提升数值稳定性，代价是每步反向传播的计算量略微增加（多做几次 QR 分解）。
...
代码里通过 `Eigen::LLT`（标准 Cholesky 分解）判断数值问题，一旦分解失败就走 `IncreaseRegularization` 增大 `mu`（也就是论文里的 $\rho$）重试，用"失败重试+自适应正则化"的工程手段来规避平方根 BP 要解决的数值稳定性问题。这是一种更简单但鲁棒性略逊的替代方案——对于本工程状态维度只有 5、控制维度只有 1 的小规模问题，这个简化通常是够用的；如果未来遇到病态问题频发，平方根 BP 会是一个值得引入的优化方向。
...
**解决方案**：人为地在动力学里加一个"虚拟控制" $w_k \in \mathbb{R}^n$（维度等于状态维度，让系统变得"完全能控/fully actuated"）：

$$
x_{k+1} = f(x_k, u_k) + w_k \qquad \text{式(36)}
$$

给定期望轨迹 $\tilde{x}$ 和某个粗糙的控制猜测 $U$，可以直接算出让状态轨迹精确匹配的 $w_k$（式37）：

$$
w_k = \tilde{x}_{k+1} - f(x_k, u_k)
$$

然后把 $w_k$ 也当作待优化的"控制量"，加入代价 $\sum \frac{1}{2} w_k^{\top} R_{\inf} w_k$（式38）并附加约束 $w_k=0$。随着优化的进行（$w_k$ 的惩罚权重 $R_{\inf}$ 通过 AL 框架不断加大，或作为等式约束被 AL 逐步逼近 0），最终 $w$ 趋近于 0，问题退化回原始的严格动力学可行问题。
...
这里的权重设计很有意思：`x_weight` 只在位置 $(x,y)$ 和 $\dot{\kappa}$ 上给了非零权重，$\theta, \kappa$ 权重为 0——这是因为参考路径的朝向和曲率往往不是"硬性目标"，只需要位置贴合参考线，让动力学自己算出合理的朝向和曲率即可，这样生成的初始路径更平滑、更符合车辆运动学。这是工程上对论文思想的灵活变通。

### 9.3 最小时间问题（Minimum Time）——论文 III.C

把每一步的时间步长 $\tau_k=\sqrt{dt_k}$ 也当作待优化的控制量之一，让求解器自己决定"每一步该花多长时间"，从而实现"用最短总时间完成任务"的优化目标（同时约束所有 $\tau_k$ 相等，避免求解器通过任意扭曲时间离散化来"作弊降低代价"）。
...
**动机**：论文指出 AL 方法有个先天缺陷——罚参数 $\mu$ 不能无限增大（数值上会爆炸），所以 AL 方法只能收敛到一个"粗糙"的容差（比如约束违反量 $10^{-3}$），如果要达到机器精度级别的约束满足（$10^{-8}$），AL 方法收敛会变得极慢。
...
$$
\delta Y_p \leftarrow \delta Y_p - H^\top (HH^\top)^{-1} h
$$
...
1. **精度需求不同**：论文的机器臂避障场景需要极高精度（因为要在物理机器人上执行，容差稍大就可能碰撞），而车辆路径规划任务通常在几何约束外还留有安全 buffer（车道边界的 `buffer`、避障的安全距离等，见 `restriction_constraint_config().buffer()` 等配置），$10^{-4}\sim10^{-3}$ 量级的约束违反完全在安全裕度内，不需要追求机器精度。
2. **工程复杂度与实时性**：投影牛顿法需要显式组装稀疏 KKT 矩阵并求解线性系统 $HH^\top$，对于车载实时控制场景（要求毫秒级求解），这个额外阶段的收益和成本不成正比。
...
2. **Car Escape（走出迷宫式障碍区）**：标准约束 iLQR（不带 AL）无法找到无碰撞路径，但 ALTRO 可以；配合投影牛顿法（ALTRO*）能把约束违反精度从 $10^{-3}$ 提升到 $10^{-8}$。
...
| $Q_{xx},Q_{uu},Q_{ux},Q_x,Q_u$ | 动作价值函数二次展开系数 | 同名变量 | `solver.h::BackwardIteration` |
| $K_k, d_k$ | 反馈增益、前馈项 | `meta->K_trajectory[i]`, `meta->k_trajectory[i]` | `solver.h::BackwardIteration` |
| $\rho$（正则化） | 保证 $Q_{uu}$ 可逆 | `meta->mu` | `solver.h` |
| $\alpha$（线搜索步长） | 前向传播步长 | `meta.alpha`, `alpha_values_` | `solver.h::Solve` |
| $\Delta V_k$ | 预期代价改进量 | `meta.dV[0]`, `meta.dV[1]` | `solver.h` |
...
| $L_A(x,\lambda,\mu)$ | 增广拉格朗日函数 | `ALCostFunction::EvaluateAugmentedLagrangian` | `al_cost_function.h` |
| $\lambda$（拉格朗日乘子） | 对偶变量 | `ConstraintPenalty::lambda_` | `constraint_penalty.h` |
| $\mu$（罚参数） | 惩罚权重 | `ConstraintPenalty::mu_` | `constraint_penalty.h` |
| $\varphi$（罚参数放大因子） | 罚参数调度系数 | `config.penalty_scaling_factor` | `al_solver.h` |
...
在状态里额外引入 $\kappa$（曲率）和 $\dot{\kappa}$（曲率变化率）作为独立的状态量（而不仅仅是 $x,y,\theta$ 三个基本量），本质上就是把"曲率变化率的变化率"$\ddot{\kappa}$（控制量）作为最高阶的自由变量，让积分出来的路径曲率天然连续、光滑变化——这正是 Clothoid 路径的核心特性，也是为什么这个状态空间设计（`kX,kY,kTheta,kKappa,kDKappa`）会被称为"clothoid path"的原因。
...
**设定**：状态 $x\in\mathbb{R}$，控制 $u\in\mathbb{R}$，一步的线性动力学 $x_{k+1} = x_k + u_k$（$A=1, B=1$），代价 $l(x,u) = x^2 + u^2$（$l_x=2x,\ l_{xx}=2,\ l_u=2u,\ l_{uu}=2,\ l_{ux}=0$），只有一步（$N=1$），终端代价 $l_f(x)=x^2$（$p_N=2x_N,\ P_N=2$）。

**第0步（终端）**：假设 $x_1=1$（这是当前迭代的名义轨迹），则：

$$
p_1 = 2\cdot x_1 = 2, \qquad P_1 = 2
$$
...
$$
\begin{aligned}
Q_x &= l_x + A\cdot p_1 = 2\cdot0.5 + 1\cdot2 = 3 \\
Q_u &= l_u + B\cdot p_1 = 2\cdot0.5 + 1\cdot2 = 3 \\
Q_{xx} &= l_{xx} + A^2\cdot P_1 = 2 + 1\cdot2 = 4 \\
Q_{uu} &= l_{uu} + B^2\cdot P_1 = 2 + 1\cdot2 = 4 \\
Q_{ux} &= l_{ux} + B\cdot P_1\cdot A = 0 + 1\cdot2\cdot1 = 2
\end{aligned}
$$

加正则化 $\rho=0$（假设数值良好不需要正则化）：

$$
\begin{aligned}
d_0 &= -(Q_{uu})^{-1}\cdot Q_u = -3/4 = -0.75 \\
K_0 &= -(Q_{uu})^{-1}\cdot Q_{ux} = -2/4 = -0.5
\end{aligned}
$$

**含义解读**：$d_0=-0.75$ 说明当前的 $u_0=0.5$ 偏大了，应该减小 0.75（前馈修正）；$K_0=-0.5$ 说明如果状态出现正向偏差 $\delta x_0$，应该反向调整控制（负反馈，这跟直觉一致——比如 $x$ 偏大就应该减小 $u$ 让下一步 $x$ 不要更大）。

**前向传播**（$\alpha=1$，假设新旧轨迹此时状态一致 $\delta x_0=0$）：

$$
\bar{u}_0 = u_0 + \alpha\cdot d_0 + K_0\cdot 0 = 0.5 - 0.75 = -0.25
$$

$$
\bar{x}_1 = \bar{x}_0 + \bar{u}_0 = 0.5 - 0.25 = 0.25
$$

**验证是否真的更优**：原代价 $J_{old} = l(0.5,0.5)+l_f(1) = (0.25+0.25)+1 = 1.5$；新代价 $J_{new} = l(0.5,-0.25)+l_f(0.25) = (0.25+0.0625)+0.0625 = 0.375$。代价从 $1.5$ 降到 $0.375$，确实下降了，说明这一步 iLQR 迭代是有效的。如果这个问题多迭代几轮，会继续收敛到解析最优解 $x_0=0.5$（固定，是初始条件的一部分，这里简化为固定值不作为变量）附近的真正最优 $u_0$。
...
**Q1：iLQR 和 DDP 有什么区别？**
DDP（Differential Dynamic Programming）在展开 $Q_k$ 时，会保留动力学函数 $f$ 的二阶导数项（$f_{xx}$ 等），而 iLQR 为了简化计算，只用动力学的一阶导数（雅可比 $A_k,B_k$），把动力学当作局部线性来处理，只有代价函数保留二阶信息。iLQR 可以看作是 DDP 的一个近似简化版本，计算更快，在大多数工程场景下精度损失可以接受。本仓库的 `solver.h` 用的正是 iLQR 简化（`DynamicModel::EvaluateJacobians` 只返回一阶雅可比 `dfdx, dfdu`，没有二阶导数接口）。

**Q2：为什么代码里既有 `mu`（iLQR 正则化）又有 AL 里的 `mu`（罚参数），这是同一个东西吗？**
不是。这是历史上两篇不同论文用了相同符号的巧合（iLQR/DDP 领域的正则化系数、AL 方法的罚参数，恰好都习惯用 $\mu$ 表示）。代码里通过命名空间区分：`Solver::IterationMeta::mu` 是 iLQR 反向传播里保证 $Q_{uu}$ 可逆的正则化系数（对应论文 $\rho$，定义在 `solver.h` 里），而 `ConstraintPenalty::mu_` 是增广拉格朗日框架里的约束惩罚权重（对应论文式 21 里的 $\mu$，定义在 `constraint_penalty.h` 里）。两者作用的层级不同：前者服务于"内层 iLQR 单次反向传播的数值稳定性"，后者服务于"外层 AL 循环里约束满足程度的调度"。阅读代码时看它属于 `Solver<...>` 还是 `ALCostFunction/ConstraintPenalty` 就能分辨。

**Q3：为什么状态里要放 $\kappa$（曲率）和 $\dot{\kappa}$（曲率变化率），而不是直接用 $x,y,\theta$ 三维状态加"转向角"作为控制？**

如果只用 $x,y,\theta$ 作为状态、"转向角"（等价于瞬时曲率）作为控制，那么优化出来的曲率序列可能是不连续跳变的（因为控制量在 iLQR 里逐步更新时没有显式的平滑性约束），对应到真实车辆就是要求方向盘瞬间打到某个角度，物理上不可实现。把 $\kappa, \dot{\kappa}$ 也纳入状态、把 $\ddot{\kappa}$ 作为控制，相当于对"曲率"这个量又做了两次积分（$\ddot{\kappa}\to\dot{\kappa}\to\kappa$），天然保证了曲率的连续性和曲率变化率的连续性（因为状态是积分出来的，积分天然连续），这正是"回旋曲线路径"名称的由来，也是第 12.1 节详细解释的内容。
...
3. 增广拉格朗日法通过"乘子 $\lambda$ + 罚参数 $\mu$"的组合，把约束优雅地转化为可以用无约束求解器（iLQR）处理的软惩罚，并通过外层迭代逐步逼近约束的精确满足；
4. ALTRO = AL 框架 + iLQR 内核 + 若干工程增强（平方根 BP、不可行轨迹初始化、最小时间、投影牛顿精修），本仓库完整实现了前两者核心思想，并结合自动驾驶路径规划场景做了大量工程化的定制（多阶段求解、动态维度约束、多候选初值竞争等）；
5. `clothoid_path_optimizer` 是这套理论在无人车路径规划领域的一个具体而完整的落地案例——状态 $[x,y,\theta,\kappa,\dot{\kappa}]$、控制 $[\ddot{\kappa}]$、约束覆盖车道边界/避障/运动学限制、代价覆盖引导线/平滑性/舒适性，通过 `ALSolver::Solve` 一行代码驱动整个 AL+iLQR 求解流程。