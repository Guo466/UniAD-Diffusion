...
factor_tagged = 1
scene_extract =  {
    'lane_keeping': 3.53 * factor_base, 
    'deviation_correction': 0.24 * factor_base, 
    'normal': 2.59 * factor_base, 
    'cross_lane_keeping': 1.05 * factor_base, 
    'acc': 5.34 * factor_base, 
    'turn_right': 0.34 * factor_base, 
    'left_change_lane': 0.52 * factor_base, 
    'near_intersection': 0.02 * factor_base, 
    'right_change_lane': 0.58 * factor_base, 
    'left_change_lane_efficient': 0.02 * factor_base, 
    # 'routing_multi_right_lane_change': 999, 
    'interactive_turn_right': 0.21 * factor_base, 
    # 'routing_multi_left_lane_change': 999, 
    'turn_left': 0.5 * factor_base, 
    'interactive_agent_large_dec': 0.84 * factor_base, 
    'static': 12.93 * factor_base, 
    'brake2stop': 1.14 * factor_base, 
    'interactive_agent_cross': 0.01 * factor_base, 
    'brake': 0.44, 'split': 0.01 * factor_base, 
    'large_curvature_lane_keeping': 0.29 * factor_base, 
    'interactive_turn_left': 0.29 * factor_base, 
    'turn_left_static': 4.02 * factor_base, 
    'interactive_turn_left_static': 0.21 * factor_base, 
    'right_change_lane_efficient': 0.01 * factor_base, 
    'merge': 0.01 * factor_base, 
    'safe_acc': 0.05 * factor_base, 
    'turn_right_static': 0.15 * factor_base, 
    'left_nudge': 0.13 * factor_base, 
    'interactive_turn_right_static': 0.01 * factor_base, 
    'right_nudge': 0.06 * factor_base, 
    'roundabout': 0.01 * factor_base
}
...
class LitDiffusionDataset(L.LightningDataModule):
    ...
    def train_dataloader(self):
        ...
        #不改没法单worker pdb
        #后边记得改掉
        prefetch = int(os.environ.get("PREFETCH_FACTOR", "4")) if self.num_workers > 0 else None
        return DataLoader(self.train_dataset, batch_size=self.batch_size, shuffle=True, num_workers=self.num_workers, collate_fn=self.train_dataset.collate_fn, pin_memory=True, persistent_workers=False, prefetch_factor=prefetch)
    
    def val_dataloader(self):
        return DataLoader(self.val_dataset, batch_size=self.batch_size, shuffle=False, num_workers=self.num_workers, collate_fn=self.val_dataset.collate_fn, pin_memory=True, persistent_workers=False)
    
    def test_dataloader(self):
        return DataLoader(self.test_dataset, batch_size=self.batch_size, shuffle=False, num_workers=self.num_workers, collate_fn=self.test_dataset.collate_fn, pin_memory=True, persistent_workers=False)
    
    def predict_dataloader(self):
        return DataLoader(self.predict_dataset, batch_size=self.batch_size, shuffle=False, num_workers=self.num_workers, collate_fn=self.predict_dataset.collate_fn, pin_memory=True, persistent_workers=False)
...
class DiffusionDataset(Dataset):
    def __init__(self, data_config, is_train=True, cache_data_to_disk=False, num_workers=4):
        ...
        self.data_dir = [os.path.join("s3://dlp_pl_prediction", d) for d in self.data_dir]

        if not cache_data_to_disk:
            self.load_data_from_aoss()
    ...
    def __getitem__(self, index):
        if self.data_config["store_data_in_memory"]:
            ...
            if ('s3:' not in data_path):
                data_path = "/".join(self.data_dir[0].split("/")[:-1]+self.processed_data_paths[index].split("/")[-5:])
            tmp = self.client.get(data_path)
    ...
    def load_data_from_aoss(self):
        print(f'load data from aoss {self.local_rank}...')
        self.processed_data_paths_dict = {}
        for cnt, dataset_dir in enumerate(self.data_dir):
            dataset_name = dataset_dir.split('/')[-1]
            data_used_num = self.data_config["data_used_num"][cnt] // self.data_chunk_size if self.data_config["data_used_num"] is not None and self.data_config["data_used_num"][cnt] is not None else None
            self.cache_data_dir = os.path.join(dataset_dir, f'cache_{dataset_name}')
            if not self.client.isdir(self.cache_data_dir):
                raise FileNotFoundError(f"Local rank {self.local_rank}: Cache data directory {self.cache_data_dir} does not exist")
            else:
                processed_data_paths_dict = self.get_data_paths_from_cache_in_aoss(data_used_num)
            ...
            if self.data_config["store_data_in_memory"]:
                ...
                for processed_data_path in processed_data_paths_dict.keys():
                    tmp = self.client.get(processed_data_path)
                    processed_data = pickle.loads(tmp)
    ...
    def collate_fn(batch):
        ...
        for sample in batch_samples:
            ...
            fix_distance_path_gt = sample["path_gt_for_loss"]
            fix_distance_path_gt = np.concatenate([fix_distance_path_gt[..., :2], fix_distance_path_gt[..., 2:3]], axis=-1)
            ego_fix_distance_path_gt.append(fix_distance_path_gt)
        ...
        model_input["ego_fix_distance_path_gt"] = _pad_first_dim(
            ego_fix_distance_path_gt, 1
        )
        model_input["ego_fix_distance_path_gt_valid"] = _pad_first_dim(
            [sample["path_gt_valid"] for sample in batch_samples], 1
        )
        ...
        with open('/mnt/afs/liangzhihui/dif_Test/dlp_diffusion_rl/input_b2.pkl', 'rb') as f:  # 注意 'rb'，表示以二进制读
            data = pickle.load(f)
        #import pdb
        #pdb.set_trace()
        #return data
        return {
            "model_input": data['raw'],
            "gt_info": data['raw'],
            "motion_metrics_input": data['raw'],
            "ego_planning_metrics_input": data['raw'],
            "submission_input": data['raw']
        }
    ...
    def get_data_paths_from_cache_in_aoss(self, data_used_num):
        '''
        Get data from cache
        Return:
            processed_data_loaded_dict: dict, {processed_data_file_path: metainfo}
        '''
        cached_data_paths_path = os.path.join(self.cache_data_dir, 'cached_data_paths.pkl')
        # NOTE: aoss client.contains has trouble with multiple processes and large files
        # if self.client.contains(cached_data_paths_path):
        #     tmp = self.client.get(cached_data_paths_path)
        #     processed_data_paths_dict = pickle.loads(tmp)
        # else:
        #     raise FileNotFoundError(f"File path list not found in {self.cache_data_dir}")
        tmp = self.client.get(cached_data_paths_path)
        processed_data_paths_dict = pickle.loads(tmp)
