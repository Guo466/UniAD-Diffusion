...
from utils.misc_batch import get_world_size
import copy
...
class EgoMotionPlanningMetric_mean_std_fix_distance_path(Metric):
    def __init__(self, is_waymo_dataset, **kwargs):
        ...
        self.add_state('ego_gt_xy', default=[], dist_reduce_fx=None)
        self.add_state('ego_gt_xy_is_valid', default=[], dist_reduce_fx=None)
        self.add_state('ego_gt_fixed_xy', default=[], dist_reduce_fx=None)
        self.add_state('ego_gt_fixed_xy_is_valid', default=[], dist_reduce_fx=None)
        self.add_state('ego_yaw', default=[], dist_reduce_fx=None)
        self.add_state('ego_fixed_yaw', default=[], dist_reduce_fx=None)
        self.metrics_config = METRICS_CONFIG
        self.is_waymo_dataset = is_waymo_dataset
        self.return_AP_details = False
        self.AV_OFFSET = 1

    def update(self, model_input: dict):
        batch_size = len(model_input['ego_status'])
        for i in range(batch_size):
            self.ego_gt_xy.append(model_input['ego_future_status'][i][:, :2])
            self.ego_gt_xy_is_valid.append(model_input['ego_future_mask'][i]) # [num_groups=1, num_agents, num_gt_steps]
            self.ego_gt_fixed_xy.append(model_input['ego_future_status_fixed'][i][:, :2]) # [40, 2]
            self.ego_gt_fixed_xy_is_valid.append(model_input['ego_future_mask_fixed'][i])
            self.ego_yaw.append(model_input['ego_future_status'][i][:, 4])
            self.ego_fixed_yaw.append(model_input['ego_future_status_fixed'][i][:, 4])
    ...
    def compute(self):
        ...
        if world_size == 1:
            ego_gt_xy = torch.stack(self._gather_to_cpu(self.ego_gt_xy), dim=0)
            ego_gt_mask = torch.stack(self._gather_to_cpu(self.ego_gt_xy_is_valid), dim=0)
            ego_gt_fixed_xy = torch.stack(self._gather_to_cpu(self.ego_gt_fixed_xy), dim=0)  # [bs, 40, 2]
            ego_gt_fixed_mask = torch.stack(self._gather_to_cpu(self.ego_gt_fixed_xy_is_valid), dim=0)  # [bs, 40]
            ego_yaw = torch.stack(self._gather_to_cpu(self.ego_yaw), dim=0)
            ego_fixed_yaw = torch.stack(self._gather_to_cpu(self.ego_fixed_yaw), dim=0)
        
        else:
            ego_gt_xy = torch.stack(self.ego_gt_xy)
            ego_gt_mask = torch.stack(self.ego_gt_xy_is_valid)
            ego_gt_fixed_xy = torch.stack(self.ego_gt_fixed_xy)
            ego_gt_fixed_mask = torch.stack(self.ego_gt_fixed_xy_is_valid)
            ego_yaw = torch.stack(self.ego_yaw)
            ego_fixed_yaw = torch.stack(self.ego_fixed_yaw)

        ego_delta_gt_mask = ego_gt_mask[:, 1:] * ego_gt_mask[:, :-1]
        ego_delta_gt_mask = torch.cat([ego_gt_mask[:, :1], ego_delta_gt_mask], dim=1)
        ego_dx_dy = ego_gt_xy[:, 1:, :] - ego_gt_xy[:, :-1, :] # [B, T_f, 2]
        ego_dx_dy = torch.cat([ego_gt_xy[:, :1, :], ego_dx_dy], dim=1)
        ego_dx_dy = ego_dx_dy * ego_delta_gt_mask[:, :, None]
        ego_delta_yaw = ego_yaw[:, 1:] - ego_yaw[:, :-1]
        ego_delta_yaw = torch.cat([ego_yaw[:, :1], ego_delta_yaw], dim=1)
        ego_delta_yaw = ego_delta_yaw * ego_delta_gt_mask

        ego_delta_gt_fixed_mask = ego_gt_fixed_mask[:, 1:] * ego_gt_fixed_mask[:, :-1]
        ego_delta_gt_fixed_mask = torch.cat([ego_gt_fixed_mask[:, :1], ego_delta_gt_fixed_mask], dim=1)
        ego_fixed_dx_dy = ego_gt_fixed_xy[:, 1:, :] - ego_gt_fixed_xy[:, :-1, :] # [B, T_f, 2]
        ego_fixed_dx_dy = torch.cat([ego_gt_fixed_xy[:, :1, :], ego_fixed_dx_dy], dim=1)
        ego_fixed_dx_dy = ego_fixed_dx_dy * ego_delta_gt_fixed_mask[:, :, np.newaxis]
        ego_fixed_delta_yaw = ego_fixed_yaw[:, 1:] - ego_fixed_yaw[:, :-1]
        ego_fixed_delta_yaw = torch.cat([ego_fixed_yaw[:, :1], ego_fixed_delta_yaw], dim=1)
        ego_fixed_delta_yaw = ego_fixed_delta_yaw * ego_delta_gt_fixed_mask
        repeat_times = ego_dx_dy.shape[1]
        metric_results_all['ego_future_status_x']['mean'] = torch.mean(ego_dx_dy[:, :, 0]).repeat(repeat_times)
        metric_results_all['ego_future_status_x']['std'] = torch.std(ego_dx_dy[:, :, 0]).repeat(repeat_times)
        metric_results_all['ego_future_status_y']['mean'] = torch.mean(ego_dx_dy[:, :, 1]).repeat(repeat_times)
        metric_results_all['ego_future_status_y']['std'] = torch.std(ego_dx_dy[:, :, 1]).repeat(repeat_times)
        metric_results_all['ego_delta_yaw']['mean'] = torch.mean(ego_delta_yaw).repeat(repeat_times)
        metric_results_all['ego_delta_yaw']['std'] = torch.std(ego_delta_yaw).repeat(repeat_times)

        # 分段统计
        metric_results_seperate['ego_future_status_x']['mean'] = torch.mean(ego_dx_dy[..., 0], dim=0) # [25]
        metric_results_seperate['ego_future_status_x']['std'] = torch.std(ego_dx_dy[..., 0], dim=0) 
        metric_results_seperate['ego_future_status_y']['mean'] = torch.mean(ego_dx_dy[..., 1], dim=0)
        metric_results_seperate['ego_future_status_y']['std'] = torch.std(ego_dx_dy[..., 1], dim=0) 
        metric_results_seperate['ego_delta_yaw']['mean'] = torch.mean(ego_delta_yaw, dim=0)
        metric_results_seperate['ego_delta_yaw']['std'] = torch.std(ego_delta_yaw, dim=0)

        fixed_repeat_times = ego_fixed_dx_dy.shape[1]
        metric_results_all['ego_future_status_fixed_x']['mean'] = torch.mean(ego_fixed_dx_dy[:, :, 0]).repeat(fixed_repeat_times)
        metric_results_all['ego_future_status_fixed_x']['std'] = torch.std(ego_fixed_dx_dy[:, :, 0]).repeat(fixed_repeat_times)
        metric_results_all['ego_future_status_fixed_y']['mean'] = torch.mean(ego_fixed_dx_dy[:, :, 1]).repeat(fixed_repeat_times)
        metric_results_all['ego_future_status_fixed_y']['std'] = torch.std(ego_fixed_dx_dy[:, :, 1]).repeat(fixed_repeat_times)
        metric_results_all['ego_fixed_delta_yaw']['mean'] = torch.mean(ego_fixed_delta_yaw).repeat(fixed_repeat_times)
        metric_results_all['ego_fixed_delta_yaw']['std'] = torch.std(ego_fixed_delta_yaw).repeat(fixed_repeat_times)
        
        # #分段统计fixed
        metric_results_seperate['ego_future_status_fixed_x']['mean'] = torch.mean(ego_fixed_dx_dy[..., 0], dim=0)
        metric_results_seperate['ego_future_status_fixed_x']['std'] = torch.std(ego_fixed_dx_dy[..., 0], dim=0)
        metric_results_seperate['ego_future_status_fixed_y']['mean'] = torch.mean(ego_fixed_dx_dy[..., 1], dim=0)
        metric_results_seperate['ego_future_status_fixed_y']['std'] = torch.std(ego_fixed_dx_dy[..., 1], dim=0)
        metric_results_seperate['ego_fixed_delta_yaw']['mean'] = torch.mean(ego_fixed_delta_yaw, dim=0)
        metric_results_seperate['ego_fixed_delta_yaw']['std'] = torch.std(ego_fixed_delta_yaw, dim=0)

        with open(f'./{ego_gt_xy.shape[0]}_mean_std_all.pkl', 'wb') as f:
            pickle.dump(metric_results_all, f)

        with open(f'./{ego_gt_xy.shape[0]}_mean_std_seperate.pkl', 'wb') as f:
            pickle.dump(metric_results_seperate, f)
        
        return metric_results_all, metric_results_seperate

    def _sync_dist(self, dist_sync_fn=None, process_group=None):
        super()._sync_dist(dist_sync_fn, process_group)
        self.ego_gt_xy = self._gather_to_cpu(self.ego_gt_xy, process_group)
        self.ego_gt_xy_is_valid = self._gather_to_cpu(self.ego_gt_xy_is_valid, process_group)
        self.ego_gt_fixed_xy = self._gather_to_cpu(self.ego_gt_fixed_xy, process_group)
        self.ego_gt_fixed_xy_is_valid = self._gather_to_cpu(self.ego_gt_fixed_xy_is_valid, process_group)
        self.ego_yaw = self._gather_to_cpu(self.ego_yaw, process_group)
        self.ego_fixed_yaw = self._gather_to_cpu(self.ego_fixed_yaw, process_group)

    def _gather_to_cpu(self, list_of_tensor, process_group=None):
        cpu_list = []
        for item in list_of_tensor:
