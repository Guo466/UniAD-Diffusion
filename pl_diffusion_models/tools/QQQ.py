...
def vis(args):
    ...
    for i, frame in enumerate(tqdm(frames, desc='Visualizing')):
        ...
        with open(pkl_file, 'rb') as f:
            ...
            # draw_heading(video_save_dir, i, data['pred_yaw'][max_idx])
            # draw_v(video_save_dir, i, data['pred_v'][max_idx])
            # draw_curvature(video_save_dir, i, data['pred_curvature'][max_idx])
...
def vis(args):
    ...
    if args.only_path_compare:
        for i in tqdm(range(n), desc='Path compare'):
            ...
            visualization_path_compare(data, data_feaNotEmb, i, video_save_dir, length=length, width=width, write_img=True)
        print(f'Path compare images saved to {video_save_dir}')
        return
    ...
    for i in tqdm(range(n)):
        ...
        # print('frame ', i)
        # print(data['pred_traj'])
        # print(data['pred_traj'][:,1] - data['ego_future_status'][:,1])
        # print('-' * 20)
        ...
        img = visualization_dataset(data, data_feaNotEmb, i, video_save_dir, 'infer', length=length, width=width, write_img=True, draw_pred=True, fix_dist=False)
        video_writer.write(img)
        img_fixed = visualization_dataset(data, data_feaNotEmb, i, video_save_dir, 'infer', length=length, width=width,write_img=False, draw_pred=True, fix_dist=True)
        video_writer_fixed.write(img_fixed)
        visualization_path_compare(data, data_feaNotEmb, i, video_save_dir, length=length, width=width, write_img=True)
        # max_idx = torch.argmax(data['pred_prob'])
        # print(i)
        # print(data['pred_traj'][max_idx,:,0])
        # print(data['pred_traj'][max_idx+1,:,0])
        # print(max_idx)
        # print(data['pred_v'][max_idx])
        # print(data['pred_v'][max_idx+1])
